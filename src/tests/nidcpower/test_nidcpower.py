...
pin_map_dut_pins = {'DUTPin1': [('DCPower1', '0'), ('DCPower3', '0')],
...
def test_get_all_nidcpower_instrument_names(standalone_tsm_context: T):
    ...
    pin_map_instrument_channel_pairs = [pair for sublist in pin_map_dut_pins.values() for pair in sublist]
    ...
    for instrument_channel_pair in instrument_channel_pairs:
        ...
        instrument_channel_pair_instances = list(
            filter(lambda instr_chan_pair: instr_chan_pair == instrument_channel_pair, pin_map_instrument_channel_pairs)
        )
...
def test_pin_to_nidcpower_session(standalone_tsm_context: T, add_instrument_sessions):
    ...
    for system_pin in pin_map_system_pins:
        pin_query_context, queried_session, queried_channel_string = tsm.pin_to_nidcpower_session(system_pin)
...
def test_pin_to_nidcpower_sessions(standalone_tsm_context: T, add_instrument_sessions):
    ...
    for pin_group in pin_map_pin_groups:
        pin_query_context, queried_sessions, queried_channel_strings = tsm.pin_to_nidcpower_sessions(pin_group)
...
def test_pins_to_nidcpower_sessions(standalone_tsm_context: T, add_instrument_sessions):
    ...
    pin_query_context, queried_sessions, queried_channel_strings = \
        tsm.pins_to_nidcpower_sessions(list(pin_map_dut_pins.keys()))
    ...
    assert len(queried_sessions) == sum([pin_map_instruments[instrument_name] for instrument_name in instrument_names])
